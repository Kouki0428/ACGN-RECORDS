...
<template>
  <div>
    ...
    <template v-if="selected">
      ...
      <TucaoBox v-if="settings.showTucao" :subject-id="selected.subject?.provider_subject_id ?? null" media-type="manga" data-anchor="tucao" />
      </div>
    </template>

    <!-- 列表视图 -->
    <div v-else>
      <StatusTabs v-model="activeStatus" category="manga" />
      <div class="grid">
        <div
        ...
        >
          <CoverImage :src="r.imageUrl" :alt="r.title" class="card-cover" :class="{ 'cover-blur': !settings.showNsfw && r.nsfw }" />
